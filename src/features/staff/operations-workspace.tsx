...
export function OperationsWorkspace() {
  ...
  async function changeStatus(booking: Booking, to: BookingStatus) {
    ...
    try {
      ...
      setMessage(`Mock 狀態已更新為「${statusLabels[to]}」。`);
    } catch {
  ...
  async function reschedule(booking: Booking) {
    ...
    try {
      ...
      setMessage("已建立隔日同時段的 Mock 改期，新舊預約可互相追查。");
    } catch {
  ...
  async function createScheduleBlock() {
    ...
    const candidate: CalendarBlock = {
      ...
      kind: blockKind,
      reason: `Fictional ${blockKind} preview`,
  ...
  return (
    <section className="operations-shell">
      <div className="staff-warning" role="note"><strong>LOCAL ROLE PREVIEW · 無正式登入</strong><span>切換角色只會改變畫面權限，不代表任何人已通過身分驗證。</span></div>
      ...
      {view === "day" ? (
        <div className="operations-two-column">
          <div className="operations-list"><div className="operations-heading"><div><p className="eyebrow">DAY / 2026-07-14</p><h2>日營運</h2></div><span>{dayTimeline.length} 個項目</span></div>{dayTimeline.map((item) => item.kind === "block" ? <article className={`operation-row block-${item.block.kind}`} key={item.id}><time>{formatTime(item.startsAt)}–{formatTime(item.endsAt)}</time><div><strong>{item.block.kind.toUpperCase()}</strong><span>{mockStaff.find((staff) => staff.id === item.staffId)?.displayName} · {item.block.reason}</span></div></article> : <BookingOperationRow key={item.id} booking={item.booking} canWrite={canWrite} onStatus={changeStatus} onReschedule={reschedule} />)}</div>
      ...
      {view === "week" ? <div className="week-grid">{weekDays.map((day) => <article key={day.date} className="week-day"><header><strong>{day.date}</strong><span>{day.bookings.length} 預約 · {day.blocks.length} 行程</span></header>{day.bookings.length + day.blocks.length === 0 ? <p>無 Mock 行程</p> : <>{day.bookings.map((booking) => <div key={booking.id}><time>{formatTime(booking.startsAt)}</time><span>{mockStaff.find((staff) => staff.id === booking.staffId)?.displayName}</span><small>{statusLabels[booking.status]}</small></div>)}{day.blocks.map((block) => <div key={block.id}><time>{formatTime(block.startsAt)}</time><span>{block.kind}</span><small>{mockStaff.find((staff) => staff.id === block.staffId)?.displayName}</small></div>)}</>}</article>)}</div> : null}

      {view === "queue" ? <div className="queue-panel"><div className="operations-heading"><div><p className="eyebrow">SAFE QUEUE</p><h2>待確認與候補</h2></div><span>不取代已確認預約</span></div>{queue.length ? queue.map((booking, index) => <article key={booking.id} className="queue-row"><span>#{index + 1}</span><div><strong>{mockCustomers.find((customer) => customer.id === booking.customerId)?.name}</strong><small>{statusLabels[booking.status]} · {booking.depositStatus === "paid" ? "Mock 已付欄位" : "無付款優先承諾"}</small></div><button type="button" disabled={!canWrite} onClick={() => changeStatus(booking, "confirmed")}>確認（遇衝突會拒絕）</button></article>) : <div className="inline-state"><strong>沒有待處理項目</strong><span>目前不需要人工處理。</span></div>}</div> : null}

      {view === "customers" ? canReadCustomers ? <div className="customer-operations-grid">{visibleCustomers.map((customer) => <article key={customer.id}><header><div><strong>{customer.name}</strong><span>{customer.phoneMasked}</span></div><small>偏好設計師：{mockStaff.find((staff) => staff.id === customer.preferredStaffId)?.displayName ?? "無"}</small></header><section><h3>服務偏好</h3>{customer.preferences?.map((item) => <p key={item}>{item}</p>) ?? <p>無</p>}</section><section><h3>技術紀錄</h3>{customer.technicalNotes?.length ? customer.technicalNotes.map((note) => <div key={note.id}><strong>{note.authorLabel}</strong><span>{note.content}</span></div>) : <p>無 Mock 技術紀錄</p>}</section><details><summary>歷史紀錄 {customer.history.length} 筆</summary>{customer.history.map((entry) => <p key={entry.id}>{entry.serviceLabel} · {entry.summary}</p>)}</details></article>)}</div> : <div className="inline-state"><strong>此角色不能查看客戶資料</strong><span>請切換 Owner、Manager、櫃台或設計師角色預覽。</span></div> : null}

      {view === "configuration" ? canStaffRole(previewRole, "settings:preview") ? <div className="configuration-grid"><section><p className="eyebrow">STAFF & ROLE</p><h2>人員與值班能力</h2>{mockStaff.map((staff) => <article key={staff.id}><div><strong>{staff.displayName}</strong><span>{staff.role} · {staff.active ? "啟用" : "停用"}</span></div><p>{staff.specialties.join(" · ")}</p><small>可服務 {staff.serviceIds.length} 項 · 每週 {staff.schedule.days.length} 個排班日</small></article>)}</section><section><p className="eyebrow">SERVICE CAPABILITY</p><h2>服務設定預覽</h2>{mockServices.map((service) => <article key={service.id}><div><strong>{service.name}</strong><span>{service.durationMinutes} 分鐘</span></div><p>{service.description}</p><small>{service.priceLabel} · 緩衝 {service.bufferBeforeMinutes}/{service.bufferAfterMinutes} 分鐘</small></article>)}</section></div> : <div className="inline-state"><strong>此角色不能查看設定預覽</strong><span>只有 Owner 或 Manager 預覽可查看服務、人員與排班設定。</span></div> : null}

      {auditLog.length ? <details className="audit-preview"><summary>查看本次 Mock 稽核紀錄（{auditLog.length}）</summary>{auditLog.map((entry) => <p key={entry}>{entry}</p>)}</details> : null}
    </section>
  );
}
