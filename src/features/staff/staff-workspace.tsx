...
import {
  ...
  workspaceTabPanelId,
  type WorkspaceTab,
} from "@/features/staff/staff-tabs";
...
function failureMessage(reason: string): string {
  if (reason === "entry_read_only") return "此時段已開始或已設為唯讀，不能再修改註記。";
...
function loadFailureMessage(reason: string): string {
  if (reason === "unauthenticated" || reason === "forbidden") {
    return failureMessage(reason);
  }
  return "目前無法讀取共用資料，請稍後重試。";
}
...
export function StaffWorkspace() {
  ...
  function handleTabKeyDown(
    ...
    const nextTab = workspaceTabAfterKey(currentTab, event.key);
    if (!nextTab) return;
    ...
    <section className="staff-workspace">
      ...
      <div className="workspace-tabs" role="tablist" aria-label="員工工作台分頁">
        {WORKSPACE_TABS.map((workspaceTab) => (
          <button
            key={workspaceTab}
            ref={(element) => { tabRefs.current[workspaceTab] = element; }}
            id={workspaceTabId(workspaceTab)}
            type="button"
            role="tab"
            aria-selected={tab === workspaceTab}
            aria-controls={workspaceTabPanelId(workspaceTab)}
            tabIndex={tab === workspaceTab ? 0 : -1}
            onClick={() => selectTab(workspaceTab)}
            onKeyDown={(event) => handleTabKeyDown(event, workspaceTab)}
          >
      ...
      <div
        id={workspaceTabPanelId("agenda")}
        role="tabpanel"
        aria-labelledby={workspaceTabId("agenda")}
        hidden={tab !== "agenda"}
      >
        <div className="agenda-layout">
          ...
          <div className="agenda-panel">
            <header><div><p className="eyebrow">{selectedDate || "未選日期"}</p><h2>{selectedDate ? formatMvpDate(selectedDate) : "請選擇日期"}</h2></div><span>{ready ? visibleEntries.length : 0} 筆</span></header>
              ...
              <div className="agenda-list">
                {visibleEntries.map((entry) => (
                  <ScheduleEntryCard
                    ...
                    onSaveNote={
                      !entry.anonymized && !isTaipeiSlotPast(entry.date, entry.time, now)
                        ? saveEntryNote
                        : undefined
                    }
                  />
      ...
      <div
        id={workspaceTabPanelId("history")}
        role="tabpanel"
      ...
      <form
        id={workspaceTabPanelId("add")}
        role="tabpanel"
        aria-labelledby={workspaceTabId("add")}
        hidden={tab !== "add"}
      ...
      >
        ...
        <div className="flow-actions"><button className="button button-secondary" type="button" disabled={busy} onClick={() => selectTab("agenda")}>返回清單</button><button className="button" type="submit" disabled={busy || loading || !ready || !selectedStaffId}>{busy ? "正在儲存…" : "儲存到共用時段"}</button></div>
      </form>
    </section>
  );
}
