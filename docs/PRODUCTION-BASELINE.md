...
接受日期：2026-08-01（Asia/Taipei）

Git authority：Epic #31、Issue #32、Decision D-017
...
| 2 | 公開入口採 Cloudflare Tunnel：`https://dumbarbershop.com` → outbound-only tunnel → `127.0.0.1`。不做 router port-forward，不直接公開 Next.js 或資料庫 port。Staging 與 Production 使用不同 hostname。 | 降低家用網路、動態 IP、CGNAT 與入站防火牆設定耦合。正式 hostname 已記錄於 #32；實際帳號與密鑰在 #36 以部署設定安全提供。 |
...
4. Production domain 是 `dumbarbershop.com`，public origin 是 `https://dumbarbershop.com`。

Issue #32 所需的非秘密 Owner 輸入均已完成。Cloudflare 帳號、付款資料、聯絡資料、Tunnel credential 與其他秘密不得寫入 Git。
...
- A validated deletion request for a future booking anonymizes the customer fields but keeps the slot occupied; it does not silently create the excluded cancellation feature.
...
Staging uses fictional data and separate values. Production fails closed when a required value is absent. Passwords, tokens and filesystem secrets are entered out of band during #36／#37. The actual domain plus accountable maintenance／alert／rollback owner are non-secret deployment identity and are recorded in Issue #32.
