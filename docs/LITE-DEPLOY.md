...
5. 安裝 cloudflared，將 `scripts/cloudflared/config.template.yml` 填入 Tunnel UUID 後放到 `C:\\Windows\\System32\\config\\systemprofile\\.cloudflared\\config.yml`，先確認 `cloudflared tunnel ingress validate`，再以系統管理員執行 `cloudflared service install`，使 Tunnel 在重開機後自動恢復。正式 DNS／Tunnel 連線僅能在 #37 Owner GO 後啟用。
